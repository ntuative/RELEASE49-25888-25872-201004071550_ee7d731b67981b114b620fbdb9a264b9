package com.sulake.core.utils
{
   ...
   public class LibraryLoader extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1404:int = 5;
      ...
      protected var var_945:int;
      
      protected var var_2288:String = "";
      
      protected var var_375:int;
      
      protected var var_528:XML;
      ...
      protected var var_322:URLRequest;
      ...
      {
         var_2287 = param1;
         dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR,var_375));
      }
      ...
      protected function prepareLibrary() : void
      {
         ...
         if(var_361 == null)
         {
            failure("Failed to find resource class \"" + _name + "\" in library " + var_322.url + "!");
            return;
         }
         try
         {
         ...
         var_528 = new XML(bytes.readUTFBytes(bytes.length));
      }
      ...
      protected function debug(param1:String) : void
      {
         ...
         if(_debug)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DEBUG,var_375));
         }
      }
      ...
      protected function loadEventHandler(param1:Event) : void
      {
         switch(param1.type)
         {
            ...
            case HTTPStatusEvent.HTTP_STATUS:
               ...
               debug("Load event STATUS " + var_375 + " for file \"" + var_16.contentLoaderInfo.url + "\"");
               break;
            case Event.UNLOAD:
               debug("Load event UNLOAD for file \"" + var_16.contentLoaderInfo.url + "\"");
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_UNLOAD,var_375));
               break;
            case ProgressEvent.PROGRESS:
               debug("Load event PROGRESS for file \"" + var_16.contentLoaderInfo.url + "\"");
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_PROGRESS,var_375));
               break;
            case IOErrorEvent.IO_ERROR:
               failure("IO Error, send or load operation failed for file \"" + var_16.contentLoaderInfo.url + "\"");
               if(!handleHttpStatus(var_375))
               {
         ...
         if(var_454 == LibraryLoader.STATE_READY)
         {
            ...
            prepareLibrary();
            removeEventListeners();
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE,var_375));
         }
      }
      ...
      protected function handleHttpStatus(param1:int) : Boolean
      {
         if(param1 >= 400)
         {
            if(var_945 > 0)
            {
               var_16.close();
               var_16.unload();
               addRequestCounterToUrlRequest(var_322,const_1404 - var_945);
               var_16.load(var_322,var_14);
               --var_945;
               return true;
            }
