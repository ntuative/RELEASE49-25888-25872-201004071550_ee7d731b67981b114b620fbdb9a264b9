package com.sulake.core.runtime.events
{
   ...
   public class EventDispatcher implements IEventDispatcher, IDisposable
   {
      ...
      private static const const_754:uint = 2;
       
      
      private var var_654:Error;
      
      protected var _disposed:Boolean = false;
      
      private var var_248:Dictionary;
      ...
      public function dispatchEvent(param1:Event) : Boolean
      {
         if(!_disposed)
         {
            ...
            if(_result == const_754)
            {
               if(var_654 != null)
               {
                  Core.crash("Error catched in EventDispatcher: undefined",var_654.errorID,var_654);
               }
               else
               {
                  Core.crash("Error catched in EventDispatcher. No error data available!",0,var_654);
               }
            }
      ...
      public function removeEventListener(param1:String, param2:Function, param3:Boolean = false) : void
      {
         ...
         if(!_disposed)
         {
            _loc4_ = var_248[param1];
            if(_loc4_)
            {
               _loc5_ = 0;
               for each(_loc6_ in _loc4_)
               {
                  if(_loc6_.callback == param2 && _loc6_.var_2271 == param3)
                  {
                     _loc4_.splice(_loc5_,1);
                     _loc6_.callback = null;
                     if(_loc4_.length == 0)
                     {
                        delete var_248[param1];
                     }
                     return;
                  }
                  _loc5_++;
      ...
      public function addEventListener(param1:String, param2:Function, param3:Boolean = false, param4:int = 0, param5:Boolean = false) : void
      {
         ...
         else
         {
            ...
            {
               if(_loc8_.callback == param2 && _loc8_.var_2271 == param3)
               {
                  return;
               }
      ...
      private function eventProcessor(param1:Event) : void
      {
         ...
         var array:Array = var_248[event.type];
         if(array)
         {
            callbacks = [];
         ...
         _result = !!event.isDefaultPrevented() ? uint(const_1044) : uint(const_481);
      }
      ...
      public function dispose() : void
      {
         ...
         if(!_disposed)
         {
            for(_loc1_ in var_248)
            {
               _loc2_ = var_248[_loc1_] as Array;
               for each(_loc3_ in _loc2_)
               {
                  _loc3_.callback = null;
               }
               delete var_248[_loc1_];
            }
            var_248 = null;
