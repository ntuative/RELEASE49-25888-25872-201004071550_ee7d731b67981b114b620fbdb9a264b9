package com.sulake.core.window.graphics
{
   ...
   public class WindowRenderer implements IWindowRenderer
   {
      ...
      private var var_1415:Rectangle;
      ...
      public function WindowRenderer(param1:ISkinContainer)
      {
         ...
         var_557 = new Array();
         var_1416 = new Point();
         var_1415 = new Rectangle();
         var_289 = new Rectangle();
         var_290 = new Dictionary(true);
         var_675 = param1;
      ...
      {
         var _loc2_:WindowRendererItem = var_290[param1] as WindowRendererItem;
      ...
      public function render() : void
      {
         ...
         while(_loc4_ < _loc2_)
         {
            _loc1_ = var_557.pop() as WindowController;
            if(!_loc1_.disposed)
            {
               _loc3_ = getWindowRendererItem(_loc1_);
               var_289.x = _loc3_.dirty.x;
               var_289.y = _loc3_.dirty.y;
               var_289.width = _loc3_.dirty.width;
               var_289.height = _loc3_.dirty.height;
               renderWindowBranch(_loc1_,_loc3_,var_289);
            }
            _loc4_++;
         }
      }
      ...
      public function dispose() : void
      {
         ...
         if(!_disposed)
         {
            _disposed = true;
            var_557 = null;
            for(_loc1_ in var_290)
            {
               delete var_290[_loc1_];
            }
            var_290 = null;
      ...
      private function renderWindowBranch(param1:WindowController, param2:WindowRendererItem, param3:Rectangle) : void
      {
         ...
         if(param1.visible)
         {
            if(getDrawLocationAndClipRegion(param1,param3,var_1416,var_1415))
            {
               if(param2.render(param1,var_1416,var_1415))
               {
      ...
      public function registerRenderable(param1:IWindow) : void
      {
         ...
         if(_loc2_ == null)
         {
            _loc2_ = new WindowRendererItem(this,_drawBufferAllocator,var_675);
            var_290[param1] = _loc2_;
      ...
      public function addToRenderQueue(param1:IWindow, param2:Rectangle, param3:uint) : void
      {
         ...
         if(getWindowRendererItem(param1).invalidate(param1,param2,param3))
         {
            ...
            var_289.x = param2.x - param1.x;
            var_289.y = param2.y - param1.y;
            var_289.width = param2.width;
            var_289.height = param2.height;
            if(param1.testParamFlag(WindowParam.const_32))
            {
               while(param1.testParamFlag(WindowParam.const_32))
               {
                  ...
                  var_289.offset(param1.x,param1.y);
                  param1 = _loc4_;
               }
            }
            getWindowRendererItem(param1).invalidate(param1,var_289,WindowRedrawFlag.const_242);
            if(var_557.indexOf(param1) == -1)
            {
               var_557.push(param1);
            }
         }
