package com.sulake.habbo.navigator.roomsettings
{
   ...
   public class RoomSettingsCtrl
   {
      ...
      private var var_1231:TextFieldManager;
      ...
      private var var_1982:ITextWindow;
      ...
      private function clearErrors() : void
      {
         ...
         this.var_1023.clearErrors();
         this.var_1231.clearErrors();
      }
      ...
      private function populateForm() : void
      {
         ...
         changePasswordField(_loc1_.method_1 == RoomSettingsData.const_115);
         Logger.log("CATEGORY ID: " + _loc1_.categoryId);
         setCategorySelection(_loc1_.categoryId);
         refreshMaxVisitors(_loc1_);
         setTag(var_918,_loc1_.tags[0]);
         setTag(var_920,_loc1_.tags[1]);
         if(var_1232)
         {
            if(_loc1_.allowPets)
            {
               this.var_1232.select();
            }
            else
            {
               this.var_1232.unselect();
            }
         }
         if(var_1237)
         {
      ...
      {
         Logger.log("[RoomSettingsCtrl.onDeleteButtonClick] " + var_411);
         _navigator.registerParameter("navigator.roomsettings.deleteroom.confirm.message","room_name",var_82.name);
         var _loc2_:ConfirmDialogView = new ConfirmDialogView(IFrameWindow(_navigator.getXmlWindow("ros_room_delete_confirm")),this,onConfirmRoomDelete,[var_411]);
         _loc2_.show();
      }
      ...
      {
         return IWindowContainer(_navigator.windowManager.createWindow(param1,"",HabboWindowType.const_59,HabboWindowStyle.const_40,HabboWindowParam.const_69,new Rectangle(0,0,100,20)));
      }
      ...
      public function refresh(param1:IWindowContainer) : void
      {
         ...
         var_1502.visible = _inRoom;
         var_1501.visible = _inRoom;
         var_1983.visible = var_501;
         var_1982.visible = !var_501;
         if(var_501)
         {
            var_1027.visible = true;
            var_1234.visible = false;
            refreshFlatControllers();
            var_1027.height = Util.getLowestPoint(this.var_1027) + 4;
         }
         else
         {
            populateForm();
            ...
            _navigator.thumbRenderer.method_4(IWindowContainer(var_1235.findChildByName("picframe")),_navigator.data.enteredGuestRoom.thumbnail,false);
            var_1025.height = Util.getLowestPoint(this.var_1025) + 4;
         }
         var_1504.text = !!var_501 ? "${navigator.roomsettings.tobasicsettings}" : "${navigator.roomsettings.toadvancedsettings}";
         Util.moveChildrenToColumn(_window,[var_1502.name,var_1235.name,var_1025.name,var_1027.name,var_1234.name,var_1501.name],0,0);
      ...
      {
         this.var_411 = param1;
         _navigator.send(new GetRoomSettingsMessageComposer(var_411));
         if(var_1024 != null)
         {
            var_1024.visible = false;
         }
      }
      ...
      private function onSaveButtonClick(param1:WindowMouseEvent) : void
      {
         ...
         switch(_loc4_.name)
         {
            case "doormode_doorbell":
               _loc2_.method_1 = RoomSettingsFlatInfo.const_151;
               break;
            case "doormode_password":
               _loc2_.method_1 = RoomSettingsFlatInfo.const_115;
               break;
            default:
               _loc2_.method_1 = RoomSettingsFlatInfo.const_372;
         }
         if(_loc2_.method_1 == RoomSettingsFlatInfo.const_115)
         {
            _loc8_ = var_1023.getText();
            _loc9_ = var_1231.getText();
            if(_loc8_ != _loc9_)
            {
               this.var_1023.clearErrors();
               this.var_1231.displayError("${navigator.roomsettings.invalidconfirm}");
               return;
            }
            if(_loc8_ != "")
            {
         ...
         _loc2_.allowPets = var_1232.isSelected;
         _loc2_.allowFoodConsume = var_1237.isSelected;
         _loc2_.allowWalkThrough = var_1236.isSelected;
         _loc2_.tags = new Array();
         addTag(var_918,_loc2_.tags);
      ...
      private function onRemoveAllFlatCtrlsClick(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         _navigator.send(new RemoveAllRightsMessageComposer(this.var_411));
      }
      
      public function onRoomSettingsSaved(param1:int) : void
      {
         if(param1 != this.var_411 || var_1026 < 1)
         {
            return;
         }
         close();
      ...
      public function onRoomSettingsSaveError(param1:int, param2:int, param3:String) : void
      {
         ...
         var_1026 = 0;
         if(param2 == RoomSettingsSaveErrorMessageParser.const_1241)
         {
         ...
         else if(param2 == RoomSettingsSaveErrorMessageParser.const_1366)
         {
         ...
         {
            setTagError(this.var_918,param3,"${navigator.roomsettings.unacceptablewords}");
         ...
         {
            setTagError(this.var_918,param3,"${navigator.roomsettings.nonuserchoosabletag}");
            setTagError(this.var_920,param3,"${navigator.roomsettings.nonuserchoosabletag}");
         }
         else if(param2 == RoomSettingsSaveErrorMessageParser.const_1185)
         {
            this.var_1023.displayError("${navigator.roomsettings.passwordismandatory}");
         }
         else
         {
      ...
      private function prepareWindow(param1:IWindowContainer) : void
      {
         ...
         _loc2_.addEventListener(WindowEvent.const_593,onDoorModePasswordSelect);
         _loc2_.addEventListener(WindowEvent.const_941,onDoorModePasswordUnselect);
         ...
         this.var_842 = new TextFieldManager(_navigator,ITextFieldWindow(_window.findChildByName("room_name")),60);
         this.var_1233 = new TextFieldManager(_navigator,ITextFieldWindow(_window.findChildByName("description")),255);
         this.var_918 = new TextFieldManager(_navigator,ITextFieldWindow(_window.findChildByName("tag1")),30);
         this.var_920 = new TextFieldManager(_navigator,ITextFieldWindow(_window.findChildByName("tag2")),30);
         this.var_1023 = new TextFieldManager(_navigator,ITextFieldWindow(_window.findChildByName("password")),30);
         this.var_1231 = new TextFieldManager(_navigator,ITextFieldWindow(_window.findChildByName("password_confirm")),30);
         this.var_1232 = ICheckBoxWindow(_window.findChildByName("allow_pets_checkbox"));
         this.var_1237 = ICheckBoxWindow(_window.findChildByName("allow_foodconsume_checkbox"));
         this.var_1236 = ICheckBoxWindow(_window.findChildByName("allow_walk_through_checkbox"));
         var_1502 = IWindowContainer(_window.findChildByName("header_container"));
         ...
         var_1027 = IWindowContainer(_window.findChildByName("advanced_settings_container"));
         var_1501 = IWindowContainer(_window.findChildByName("switch_view_container"));
         var_1234 = IWindowContainer(_window.findChildByName("footer_container"));
         var_259 = IWindowContainer(_window.findChildByName("flat_controllers_container"));
         var_1024 = IWindowContainer(_window.findChildByName("password_container"));
         var_1982 = ITextWindow(_window.findChildByName("basic_caption"));
         var_1983 = ITextWindow(_window.findChildByName("advanced_caption"));
         var_1504 = ITextWindow(_window.findChildByName("switch_view_text"));
         var_1504.procedure = onSwitchView;
         var_1024.visible = false;
         if(!_inRoom)
         {
