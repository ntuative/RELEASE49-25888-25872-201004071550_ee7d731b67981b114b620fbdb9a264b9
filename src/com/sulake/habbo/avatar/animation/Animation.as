package com.sulake.habbo.avatar.animation
{
   ...
   public class Animation implements IAnimation
   {
      ...
      private var var_1426:AvatarDataContainer;
      
      private var _frames:Array;
      
      private var var_1425:DirectionDataContainer;
      
      private var var_1427:ShadowDataContainer;
      ...
      public function Animation(param1:AvatarStructure, param2:XML)
      {
         ...
         if(param2.hasOwnProperty("sprite"))
         {
            ...
            {
               var_1143.push(new SpriteDataContainer(this as IAnimation,_loc5_));
            }
         }
         if(param2.hasOwnProperty("avatar"))
         {
            var_1426 = new AvatarDataContainer(param2.avatar[0]);
         }
         if(param2.hasOwnProperty("canvas"))
         {
            for each(_loc6_ in param2.canvas)
            {
               var_1142.push(new CanvasDataContainer(_loc6_));
            }
         }
         if(param2.hasOwnProperty("direction"))
         {
            var_1425 = new DirectionDataContainer(param2.direction[0]);
         }
         if(param2.hasOwnProperty("shadow"))
         {
            var_1427 = new ShadowDataContainer(param2.shadow[0]);
         }
         if(param2.hasOwnProperty("remove"))
         {
            for each(_loc7_ in param2.remove)
            {
               var_1141.push(String(_loc7_.@id));
            }
         }
         if(param2.hasOwnProperty("add"))
         {
            ...
            {
               var_656.push(new AddDataContainer(_loc8_));
            }
         }
         var _loc3_:int = 0;
         for each(_loc4_ in param2.frame)
         {
            ...
            for each(_loc11_ in _loc4_.bodypart)
            {
               ...
               _loc13_ = new AnimationLayerData(param1,_loc11_,AnimationLayerData.const_724,_loc3_,_loc10_);
               _loc9_.push(_loc13_);
            }
            for each(_loc12_ in _loc4_.fx)
            {
               _loc10_ = param1.getActionDefinition(String(_loc12_.@action));
               _loc14_ = new AnimationLayerData(param1,_loc12_,AnimationLayerData.const_405,_loc3_,_loc10_);
               _loc9_.push(_loc14_);
               if(_loc10_ != null)
               {
               }
      ...
      public function getLayerData(param1:int, param2:String) : AnimationLayerData
      {
         ...
         for each(_loc3_ in getFrame(param1))
         {
            ...
            if(_loc3_.type == AnimationLayerData.const_405)
            {
               for each(_loc4_ in var_656)
               {
      ...
      public function getAnimatedBodyPartIds(param1:int) : Array
      {
         ...
         for each(_loc3_ in getFrame(param1))
         {
            if(_loc3_.type == AnimationLayerData.const_724)
            {
