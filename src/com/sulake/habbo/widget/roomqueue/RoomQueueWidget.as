package com.sulake.habbo.widget.roomqueue
{
   ...
   public class RoomQueueWidget extends RoomWidgetBase
   {
      ...
      private function changeQueue(param1:WindowMouseEvent) : void
      {
         ...
         if(var_1431 == RoomWidgetRoomQueueUpdateEvent.const_284)
         {
            _loc2_ = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.CHANGE_TO_SPECTATOR_QUEUE);
         }
         else
         {
            _loc2_ = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_677);
         }
      ...
      private function onQueueStatus(param1:RoomWidgetRoomQueueUpdateEvent) : void
      {
         ...
         if(param1.isActive)
         {
            var_1431 = param1.type;
            var_968 = false;
            var_969 = param1.position;
         }
         else
         {
            var_968 = true;
         }
         var_1730 = param1.hasHabboClub;
         var_1432 = param1.isClubQueue;
         localizations.registerParameter("room.queue.position","position",var_969.toString());
         localizations.registerParameter("room.queue.position.hc","position",var_969.toString());
         localizations.registerParameter("room.queue.spectator.position","position",var_969.toString());
         localizations.registerParameter("room.queue.spectator.position.hc","position",var_969.toString());
         showInterface();
      }
      ...
      override public function registerUpdateEvents(param1:IEventDispatcher) : void
      {
         ...
         param1.addEventListener(RoomWidgetRoomQueueUpdateEvent.const_284,onQueueStatus);
         param1.addEventListener(RoomWidgetRoomQueueUpdateEvent.const_439,onQueueStatus);
      ...
      private function exitQueue(param1:WindowMouseEvent) : void
      {
         ...
         var _loc2_:RoomWidgetRoomQueueMessage = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_537);
         messageListener.processWidgetMessage(_loc2_);
         removeWindow();
      }
      ...
      override public function unregisterUpdateEvents(param1:IEventDispatcher) : void
      {
         ...
         param1.removeEventListener(RoomWidgetRoomQueueUpdateEvent.const_284,onQueueStatus);
         param1.removeEventListener(RoomWidgetRoomQueueUpdateEvent.const_439,onQueueStatus);
      }
      ...
      private function showInterface() : void
      {
         ...
         if(_loc1_ != null && _loc2_ != null && _loc3_ != null)
         {
            switch(var_1431)
            {
               case RoomWidgetRoomQueueUpdateEvent.const_284:
                  _loc1_.caption = !!var_1432 ? "${room.queue.position.hc}" : "${room.queue.position}";
                  _loc2_.caption = "${room.queue.spectatormode}";
                  _loc3_.visible = var_968;
                  break;
               case RoomWidgetRoomQueueUpdateEvent.const_439:
                  _loc1_.caption = !!var_1432 ? "${room.queue.spectator.position.hc}" : "${room.queue.spectator.position}";
                  _loc2_.caption = "${room.queue.back}";
                  _loc3_.visible = false;
            }
            _loc2_.visible = var_968;
         }
         var _loc4_:IWindowContainer = _window.findChildByName("club_container") as IWindowContainer;
         if(_loc4_ != null)
         {
            _loc4_.visible = !var_1730;
         }
